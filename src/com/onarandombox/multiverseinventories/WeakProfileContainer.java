...
/** Implementation of ProfileContainer using WeakHashMaps to keep memory usage
 * to a minimum. */
abstract class WeakProfileContainer implements ProfileContainer {
	...
	public PlayerProfile getPlayerData(Player player) {
		ProfileType type;
		if(this.inventories.getMVIConfig().isUsingGameModeProfiles()) {
	...
	public PlayerProfile getPlayerData(ProfileType profileType, OfflinePlayer player) {
		...
		if(playerProfile == null) {
			playerProfile = this.getData().getPlayerData(this.type, this.getDataName(), profileType, player.getName());
			Logging.finer("[%s - %s - %s - %s] not cached, loading from disk...", profileType, this.type, playerProfile.getContainerName(), player.getName());
