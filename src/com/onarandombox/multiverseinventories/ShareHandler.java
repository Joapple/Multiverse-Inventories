...
/** Abstract class for handling sharing of data between worlds and game
 * modes. */
abstract class ShareHandler {
	...
	public ShareHandler(Inventories inventories, Player player, Cause cause, String fromWorld, String toWorld, GameMode fromGameMode, GameMode toGameMode) {
	...
	public final void addFromProfile(ProfileContainer container, Shares shares, PlayerProfile profile) {
		this.event.getFromProfiles().add(new DefaultPersistingProfile(shares, profile));
	}
	...
	public final void addToProfile(ProfileContainer container, Shares shares, PlayerProfile profile) {
		this.event.getToProfiles().add(new DefaultPersistingProfile(shares, profile));
	}
	
	/** Finalizes the transfer from one world to another. This handles the
	 * switching
	...
	void completeSharing() {
		Logging.finer("Change affected by " + this.event.getFromProfiles().size() + " fromProfiles and " + this.event.getToProfiles().size() + " toProfiles");
		// This if statement should never happen, really.
		if(this.event.getToProfiles().isEmpty()) {
			if(this.hasBypass) {
				Logging.fine(this.event.getPlayer().getName() + " has bypass permission for 1 or more world/groups!");
			} else {
			...
			Logging.finer("=== " + this.event.getPlayer().getName() + "'s " + this.event.getCause() + " handling complete! ===");
			return;
		}
		
		for(PersistingProfile persistingProfile : this.event.getFromProfiles()) {
			updateProfile(this.inventories, this.event.getPlayer(), persistingProfile);
		}
		for(PersistingProfile persistingProfile : this.event.getToProfiles()) {
			updatePlayer(this.inventories, this.event.getPlayer(), persistingProfile);
		}
		Logging.finer("=== " + this.event.getPlayer().getName() + "'s " + this.event.getCause() + " handling complete! ===");
	}
	...
	static void updateProfile(final Inventories inventories, final Player player, final PersistingProfile profile) {
		...
		if(debug > 0) {
			Logging.finer("Persisted: " + persisted.toString() + " to " + profile.getProfile().getContainerType() + ":" + profile.getProfile().getContainerName() + " (" + profile.getProfile().getProfileType() + ")" + " for player " + profile.getProfile().getPlayer().getName());
		}
	...
	static void updatePlayer(final Inventories inventories, final Player player, final PersistingProfile profile) {
		...
		if(!loaded.toString().isEmpty()) {
			Logging.finer("Updated: " + loaded.toString() + " for " + profile.getProfile().getPlayer().getName() + " for " + profile.getProfile().getContainerType() + ":" + profile.getProfile().getContainerName() + " (" + profile.getProfile().getProfileType() + ")");
		}
		if(!defaulted.toString().isEmpty()) {
			Logging.finer("Defaulted: " + defaulted.toString() + " for " + profile.getProfile().getPlayer().getName() + " for " + profile.getProfile().getContainerType() + ":" + profile.getProfile().getContainerName() + " (" + profile.getProfile().getProfileType() + ")");
		}
	}
}
