...
public class AdventureListener implements Listener {
	...
	/** @param event The Multiverse-Adventure event to handle when a world has
	 *            finished resetting. */
	@EventHandler
	public void worldReset(MVAResetFinishedEvent event) {
		WorldProfile worldProfile = this.inventories.getWorldManager().getWorldProfile(event.getWorld());
		for(OfflinePlayer player : this.inventories.getServer().getOfflinePlayers()) {
