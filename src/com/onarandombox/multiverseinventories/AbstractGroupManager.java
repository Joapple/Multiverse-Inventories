...
abstract class AbstractGroupManager implements GroupManager {
	...
	public List<WorldGroupProfile> getGroups() {
		return Collections.unmodifiableList(new ArrayList<>(this.getGroupNames().values()));
	}
	...
	public List<WorldGroupProfile> getGroupsForWorld(String worldName) {
		...
		List<WorldGroupProfile> worldGroups = new ArrayList<>();
	...
	public List<GroupingConflict> checkGroups() {
		...
		Map<WorldGroupProfile, WorldGroupProfile> previousConflicts = new HashMap<>();
			...
			for(String worldName : checkingGroup.getWorlds()) {
				for(WorldGroupProfile worldGroup : this.getGroupsForWorld(worldName)) {
					...
					if(!conflictingShares.isEmpty()) {
						if(checkingGroup.getWorlds().containsAll(worldGroup.getWorlds()) || worldGroup.getWorlds().containsAll(checkingGroup.getWorlds())) {
							continue;
						}
						conflicts.add(new DefaultGroupingConflict(checkingGroup, worldGroup, Sharables.fromShares(conflictingShares)));
					}
				}
			}
		}
