...
public class EnderChestListenerEarly1_3_1_RBs implements Listener {
	...
	public void inventoryOpen(final InventoryOpenEvent event) {
		...
		final List<WorldGroupProfile> groupsForWorld = this.plugin.getGroupManager().getGroupsForWorld(world);
		...
		if(multiple && !this.hasBeenWarned) {
		...
		if(playerProfile == null) {
			playerProfile = this.plugin.getWorldManager().getWorldProfile(world).getPlayerData(player);
		}
		...
		this.openInventories.put(player.getName(), event.getInventory());
	...
	public void playerQuit(final PlayerQuitEvent event) {
		...
		if(this.openInventories.containsKey(player.getName())) {
			final Inventory inventory = this.openInventories.get(player.getName());
			final String world = player.getWorld().getName();
			PlayerProfile playerProfile = null;
			final List<WorldGroupProfile> groupsForWorld = this.plugin.getGroupManager().getGroupsForWorld(world);
			for(WorldGroupProfile worldGroupProfile : groupsForWorld) {
