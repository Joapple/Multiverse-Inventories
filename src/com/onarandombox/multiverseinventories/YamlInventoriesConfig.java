...
class YamlInventoriesConfig implements InventoriesConfig {
	...
	public enum Path {
		...
		FIRST_RUN("settings.first_run", Boolean.TRUE, "# If this is true it will generate world groups for you based on MV worlds."),
		/** First Run flag config path, default and comments. */
		USE_BYPASS("settings.use_bypass", Boolean.FALSE, "# If this is set to true, it will enable bypass permissions (Check the wiki for more info.)"),
		
		/** Whether or not to make ungrouped worlds use the default group. */
		DEFAULT_UNGROUPED_WORLDS("settings.default_ungrouped_worlds", Boolean.FALSE, "# If set to true, any world not listed in a group will automatically use the settings for the default group!"),
		...
		/** First Run flag config path, default and comments. */
		OPTIONAL_SHARES("shares.use_optionals", new ArrayList<String>(), "# You must specify optional shares you wish to use here or they will be ignored.", "# The only built in optional share is \"economy\""),
	...
	public List<WorldGroupProfile> getWorldGroups() {
		return this.plugin.getGroupManager().getGroups();
	}
	...
	public void setFirstRun(boolean firstRun) {
		this.getConfig().set(Path.FIRST_RUN.getPath(), Boolean.valueOf(firstRun));
	}
	...
	public void setUsingBypass(boolean useBypass) {
		this.getConfig().set(Path.USE_BYPASS.getPath(), Boolean.valueOf(useBypass));
	}
	...
	public void setUsingLoggingSaveLoad(boolean useLoggingSaveLoad) {
		this.getConfig().set(Path.LOGGING_SAVE_LOAD.getPath(), Boolean.valueOf(useLoggingSaveLoad));
	}
