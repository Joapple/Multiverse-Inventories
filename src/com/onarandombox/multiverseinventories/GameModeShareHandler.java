...
final class GameModeShareHandler extends ShareHandler {
	...
	public GameModeShareHandler(Inventories inventories, Player player, GameMode fromGameMode, GameMode toGameMode) {
		super(inventories, player, Cause.GAME_MODE_CHANGE, player.getWorld().getName(), player.getWorld().getName(), fromGameMode, toGameMode);
	}
	
	@Override
	public void handle() {
		Player player = this.event.getPlayer();
		ProfileType fromType = ProfileTypes.forGameMode(this.event.getFromGameMode());
		ProfileType toType = ProfileTypes.forGameMode(this.event.getToGameMode());
		String world = this.event.getPlayer().getWorld().getName();
		Logging.finer("=== " + player.getName() + " changing game mode from: " + fromType + " to: " + toType + " for world: " + world + " ===");
